...
class IsoMaxPlusLossFirstPart(nn.Module):
    ...
    def forward(self, features, return_mds=False):
        f_hat = F.normalize(features)
        p_hat = F.normalize(self.prototypes)
        distances = torch.cdist(
            f_hat, p_hat, p=2.0, compute_mode="donot_use_mm_for_euclid_dist"
        )
        mds = torch.min(distances, dim=1)[0]
        distances = distances * torch.abs(self.distance_scale)
...
class IsoMaxPlusLossSecondPart(nn.Module):
    ...
    def __init__(self, entropic_scale=10.0, reduce=True):
    ...
    def forward(self, logits, targets, debug=False):
        ...
        probabilities_at_targets = probabilities_for_training[
            range(distances.size(0)), targets
        ]
        ...
        if not debug:
            ...
            intra_inter_distances = torch.where(
                targets_one_hot != 0, distances, torch.Tensor([float("Inf")]).cuda()
            )
            inter_intra_distances = torch.where(
                targets_one_hot != 0, torch.Tensor([float("Inf")]).cuda(), distances
            )
            intra_distances = intra_inter_distances[
                intra_inter_distances != float("Inf")
            ]
            inter_distances = inter_intra_distances[
                inter_intra_distances != float("Inf")
            ]
