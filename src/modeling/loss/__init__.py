...
class GCELoss(nn.Module):
    ...
    def forward(self, pred, labels):
        ...
        label_one_hot = F.one_hot(labels, self.num_classes).float().to(pred.device)
        loss = (
            1.0 - torch.pow(torch.sum(label_one_hot * pred, dim=1), self.q)
        ) / self.q
        return loss.mean()
...
class AGCELoss(nn.Module):
    def __init__(self, num_classes=2, a=1, q=2, eps=eps, scale=1.0):
    ...
    def forward(self, pred, labels):
        ...
        loss = (
            (self.a + 1) ** self.q
            - torch.pow(self.a + torch.sum(label_one_hot * pred, dim=1), self.q)
        ) / self.q
...
class ANormLoss(nn.Module):
    ...
    def forward(self, pred, labels):
        ...
        loss = (
            torch.sum(
                torch.pow(torch.abs(self.a * label_one_hot - pred), self.p), dim=1
            )
            - (self.a - 1) ** self.p
        )
...
class NormalizedFocalLoss(torch.nn.Module):
    def __init__(
        self, gamma=0.5, num_classes=10, alpha=None, size_average=True, scale=1.0
    ):
...
class NFLandRCE(torch.nn.Module):
    def __init__(
        self, size_average=True, alpha=1.0, beta=1.0, num_classes=10, gamma=0.5
    ):
        super(NFLandRCE, self).__init__()
        self.num_classes = num_classes
        self.size_average = size_average
        self.nfl = NormalizedFocalLoss(
            gamma=gamma, num_classes=num_classes, size_average=size_average, scale=alpha
        )
        if size_average:
            self.rce = RCELoss(num_classes=num_classes, scale=beta)
        else:
...
class NFLandMAE(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=1.0, num_classes=2, gamma=0.5):
        super(NFLandMAE, self).__init__()
        self.num_classes = num_classes
        self.nfl = NormalizedFocalLoss(
            gamma=gamma, num_classes=num_classes, scale=alpha
        )
        self.mae = MAELoss(num_classes=num_classes, scale=beta)
...
class NCEandAGCE(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=1.0, num_classes=2, a=3, q=1.5):
...
class NCEandAUE(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=1.0, num_classes=2, a=6, q=1.5):
...
class NCEandAEL(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=4.0, num_classes=2, a=2.5):
...
class NFLandAGCE(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=1.0, num_classes=2, a=3, q=2):
...
class NFLandAUE(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=1.0, num_classes=2, a=1.5, q=0.9):
...
class NFLandAEL(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=1.0, num_classes=2, a=3):
...
class ANLandRCE(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=1.0, num_classes=2, a=2, p=0.7):
